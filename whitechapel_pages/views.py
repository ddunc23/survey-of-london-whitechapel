...
def site_home(request):
	...
	images = Image.objects.filter(published=True, author__is_staff=False).exclude(created=None).order_by('-date_published')[:5]
	documents = Document.objects.filter(published=True, author__is_staff=False).exclude(created=None).order_by('-date_published')[:5]
	media = Media.objects.filter(published=True, author__is_staff=False).exclude(created=None).order_by('-date_published')[:5]
